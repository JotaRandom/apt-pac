...
def download_source(package_name, verbose=False):
    ...
    """
    if verbose:
        console.print(f"[dim]Checking for pkgctl availability...[/dim]")
...
def parse_pkgbuild_info(pkgbuild_path):
    ...
    vars_to_extract = ["pkgname", "pkgver", "pkgrel", "pkgdesc", "arch", "url", "license", "depends", "makedepends"]
    
    # Build bash command to extract all variables
    bash_commands = []
    for var in vars_to_extract:
        bash_commands.append(f"echo '{var}:' && printf '%s\\n' \"${{{var}[@]}}\" && echo '---'")
    
    cmd = [
        "bash", "-c",
        f"source '{pkgbuild_path}' && " + " && ".join(bash_commands)
    ]
...
def handle_build_dep(package_name, verbose=False):
    ...
    try:
        if verbose:
            console.print(f"[dim]Installing build dependencies...[/dim]")
...
def handle_showsrc(package_name, verbose=False):
    ...
    if verbose:
        console.print(f"[dim]Parsing PKGBUILD info...[/dim]")
    ...
    console.print(f"[bold]Version:[/bold] {info.get('pkgver', 'unknown')}-{info.get('pkgrel', '1')}")
    ...
    if 'license' in info:
        licenses = info['license'] if isinstance(info['license'], list) else [info['license']]
    ...
    if 'depends' in info:
        deps = info['depends'] if isinstance(info['depends'], list) else [info['depends']]
        console.print(f"[bold]Depends:[/bold] {', '.join(deps)}")
    
    if 'makedepends' in info:
        makedeps = info['makedepends'] if isinstance(info['makedepends'], list) else [info['makedepends']]
