    ...
    def install(
        ...
        for i, pkg in enumerate(build_queue):
            ...
            if needed_by_future:
                ...
                for f in built_files:
                    ...
                    ui.console.print(
                        f"[bold cyan]Hit:[/bold cyan]1 [blue]{dir_path}[/blue] {pkg_display_name}",
                        highlight=False,
                    )

                ui.console.print(
                    f"[dim]{_('Installing intermediate dependency')} {pkg_name}...[/dim]"
                )
                cmd = ["pacman", "-U", "--noconfirm", "--asdeps"] + [
                    str(f) for f in built_files
                ]
        ...
        if final_batch_paths:
            ...
            for i, f in enumerate(final_batch_paths, 1):
                ...
                dir_path = f"file://{f.parent}/"
                ui.console.print(
                    f"[bold cyan]Hit:[/bold cyan]{i} [blue]{dir_path}[/blue] {pkg_display_name}",
                    highlight=False,
                )
            ...
            if run_pacman_with_apt_output(cmd, show_hooks=True):
                ui.console.print(
                    f"[success]{_('Successfully installed')} {', '.join(final_batch_names)}[/success]"
                )
            else:
