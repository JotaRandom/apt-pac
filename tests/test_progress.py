...
class TestProgressBar(unittest.TestCase):
    ...
    def test_run_pacman_progress_bar_candy(self, mock_popen, mock_run):
        ...
        with patch("apt_pac.commands.Progress") as MockProgress:
            ...
            # Verify subprocess.run was called for sync (if success)
            # Depending on implementation, it might be called.
            # verify call arguments if needed
            # mock_run.assert_called_with(["sync"], check=False)

            # Verify Progress initialized with correct columns
            # TextColumn(desc), TextColumn(counter), CandyBarColumn, TextColumn(pct)
            ...
            # Check calls for progress N/M
            progress_updates = [c for c in calls if "completed" in c.kwargs]
