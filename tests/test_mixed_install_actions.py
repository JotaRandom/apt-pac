    ...
    def test_mixed_install_sequence(self):
        ...
        def side_effect_get_aur_info(pkgs):
            if pkgs[0] == "aur_pkg":
                return [
                    {"Name": "aur-pkg"}
                ]  # Typo in commands.py usage? No it passes list.
            if pkgs[0] == "aur-pkg":
                return [{"Name": "aur-pkg"}]
            return []
        ...
        self.assertTrue(
            len(pacman_calls) > 0, "Pacman -S should be called for official pkg"
        )
        self.assertTrue(
            len(installer_calls) > 0, "Installer.install should be called for aur pkg"
        )
        ...
        print(
            "Test Mixed Install: Pacman called with official, Installer called with AUR."
        )
