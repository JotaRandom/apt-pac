...
from apt_pac import aur, sources
...
class TestAurFeatures(unittest.TestCase):
    ...
    def test_check_updates(
        self, mock_get_info, mock_vercmp, mock_get_aur_pkgs, mock_get_pkgs
    ):
    ...
    def test_download_aur_source(self, mock_run, mock_exists):
        ...
        with patch("apt_pac.config.get_config") as mock_config:
            ...
            # Run
            aur.download_aur_source("test-pkg")
    ...
    def test_smart_providers_aur(self, mock_run):
        ...
        with (
            patch.object(aur.AurResolver, "resolve", return_value=[{"Name": "foo"}]),
            ...
            for call_val in mock_run.call_args_list:
                ...
                if args and args[0] == "makepkg":
                    ...
                    found = True
            self.assertTrue(
                found, "makepkg should have been called without --noconfirm"
            )
    ...
    def test_gpg_import(self, mock_input):
        ...
        with patch("apt_pac.aur.subprocess.run") as mock_run:
            ...
            error_output = (
                b"Verifying source file signatures with gpg...\n"
                ...
                if "makepkg" in cmd:
                    ...
                    if not hasattr(side_effect, "failed_once"):
                        ...
                        raise subprocess.CalledProcessError(
                            1, cmd, output=b"", stderr=error_output
                        )
                    else:
            ...
            with (
                ...
                with patch("os.getuid", return_value=1000, create=True):
                    installer._build_pkg(
                        {"Name": "package-query", "Version": "1.9"},
                        verbose=False,
                        auto_confirm=True,
                    )
            ...
            self.assertTrue(
                gpg_called,
                "gpg --recv-keys should have been called for D1483FA6C3C07136",
            )
    ...
    def test_key_output(self, mock_console, mock_config, mock_run, mock_stdout):
        ...
        mock_run.assert_called_with(
            ["pacman-key", "--add", "key.gpg"], check=True, capture_output=True
        )
...
class TestPrivileges(unittest.TestCase):
    ...
    def test_system_install_no_root(
        ...
        with self.assertRaises(SystemExit) as cm:
            apt_commands.execute_command("install", ["foo"])
    ...
    def test_get_privilege_command(self, mock_which, mock_config):
        ...
        mock_conf_obj.get.side_effect = (
            lambda s, k, d: "sudo" if k == "privilege_tool" else d
        )
        ...
        mock_conf_obj.get.side_effect = (
            lambda s, k, d: "doas" if k == "privilege_tool" else d
        )
        cmd = get_privilege_command("user", ["cmd"])
        ...
        mock_conf_obj.get.side_effect = (
            lambda s, k, d: "run0" if k == "privilege_tool" else d
        )
        cmd = get_privilege_command("user", ["cmd"])
        ...
        mock_conf_obj.get.side_effect = (
            lambda s, k, d: "auto" if k == "privilege_tool" else d
        )
    ...
    def test_aur_build_as_root_with_sudo_user(
        self, mock_getuid, mock_run, mock_download
    ):
        from apt_pac.aur import AurInstaller
        ...
        with patch("apt_pac.aur.get_config") as mock_config:
            ...
            with (
                patch.object(
                    Path, "glob", return_value=[Path("test-pkg-1.0-1-any.pkg.tar.zst")]
                ),
                patch("apt_pac.commands.run_pacman_with_apt_output", return_value=True),
            ):
        ...
        self.assertTrue(
            found_sudo, "Should have dropped privileges with sudo -u testuser"
        )
...
class TestEasterEggs(unittest.TestCase):
    ...
    def test_moo(self, mock_getuid, mock_console):
        ...
        apt_commands.execute_command("moo", [])
    ...
    def test_pacman(self, mock_getuid, mock_console):
        ...
        apt_commands.execute_command("pacman", [])
