...
class TestNewsCommand(unittest.TestCase):
    ...
    def test_news_fetch_success_filtering(self, mock_urlopen, mock_console):
        ...
        items_xml = self.item_template.format(
            title="Recent News", date=recent_date, link="http://example.com/1", desc="Desc 1"
        )
        items_xml += self.item_template.format(
        ...
        with patch("apt_pac.commands.shutil.which", return_value=False), \
    ...
    def test_news_fallback_to_latest(self, mock_urlopen, mock_console):
        ...
        old_date_1 = (now - timedelta(days=200)).strftime("%a, %d %b %Y %H:%M:%S %z") # Latest of the old
        old_date_2 = (now - timedelta(days=300)).strftime("%a, %d %b %Y %H:%M:%S %z")
        
        items_xml = self.item_template.format(
            title="Old News Leet", date=old_date_1, link="http://example.com/1", desc="Desc 1"
        )
        items_xml += self.item_template.format(
            title="Ancient News", date=old_date_2, link="http://example.com/2", desc="Desc 2"
        )
        ...
        self.assertFalse(found_ancient, "Should NOT show ancient news if fallback selects only the latest")
